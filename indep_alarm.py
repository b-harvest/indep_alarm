...
async def shutdown(signal, loop, stop_event):
...
async def server():
  ...
  loop.run_in_executor(None, main, stop_event)

  # 비동기 웹 서버 실행
  app = await init_app()
  runner = web.AppRunner(app)
  await runner.setup()
  site = web.TCPSite(runner, '0.0.0.0', 58080)
  await site.start()
  print("Server started at http://0.0.0.0:58080")
